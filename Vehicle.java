...
public abstract class Vehicle extends SuperSmoothMover
{
    ...
    //lane change alrogithm (1 is up, -1 is down)
    public boolean changeLanes(int theDirection) {
        VehicleWorld vw = (VehicleWorld)getWorld(); 
        
        if (theDirection == 1 && myLaneNumber == 0) {
            return false;
        }
        else if (theDirection == -1 && (myLaneNumber+ 1) == vw.getLaneCount()) {
            return false;
        }
            ...
            if (availableLane(theDirection)) {
                setLocation(getX(), vw.getLanePosY()[myLaneNumber - theDirection] - yOffset);
                canChangeLane = false;
                return true;
            }
            else {
     ...
     */
    // checks for available lane (1 is up, -1 is down)
    public boolean availableLane(int theDirection) {
        int hitbox = 0;
        VehicleWorld vw = (VehicleWorld)getWorld();
        if (theDirection == 1) {
            hitbox = vw.getLaneY(myLaneNumber - theDirection);
        }
        else {
            hitbox = vw.getLaneY(myLaneNumber - theDirection);
        }
        ArrayList<Vehicle> vl = (ArrayList<Vehicle>) (getWorld().getObjects(Vehicle.class));
        int leftX = getX() - (int) (getImage().getWidth()/2 - 15 - speed); 
        int rightX = getX() + (int) (getImage().getWidth()/2 +15 + speed); 
        int topY = hitbox + getImage().getHeight()/2;
        int botY = hitbox - getImage().getHeight()/2; 
        for (Vehicle v : vl) {
            if (v != this && leftX < v.getX() && v.getX() < rightX && v.getY() < topY && v.getY() > botY) {
                return false;
            }
        }
        return true; 
    ...
    public void drive() 
    {
        ...
        ArrayList<Vehicle> ahead = (ArrayList<Vehicle>) getObjectsAtOffset (direction * (int)(speed + getImage().getWidth()/2 + 6), 0, Vehicle.class);
        double otherVehicleSpeed = -1;
        for (Vehicle infront : ahead) {
            if (infront != null) {
                otherVehicleSpeed = infront.getSpeed();
