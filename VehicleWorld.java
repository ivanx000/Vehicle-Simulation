...
/**
 * Key Features:
 * - Robbers target the pedestrians and rob them
 * - Cops target the robbers and arrest them
 * - Gray cars pick up robbers and speed away
 * - Cop cars have the chance to spawn a cop
 * - Night time effect spawns robbers and make them move faster
 * - Bomb destroys vehicles, kills cops and robbers, knocks down pedestrians
 * 
 * Credits:
 *
 * Images:
 * - Background: https://www.freepik.com/free-vector/ghetto-street-night-slum-houses-old-buildings-with-glow-windows-graffiti-walls-dilapidated-dwellings-stand-roadside-with-lamps-fire-hydrants-litter-bins-cartoon-vector-illustration_6822917.htm#
 * - Ambulance: https://pngtree.com/so/ambulance-cartoon
 * - Car: https://www.kindpng.com/imgv/TmRmJRJ_image-cartoon-car-cartoon-transparent-background-car-png/
 * - Police car: https://pngtree.com/so/cartoon-police-car
 * - Robber car / Gray car: https://png.toolxox.com/vhv?p=hbwmwmR_car-png-cartoon-car-side-view-clipart-transparent/
 * Sounds:
 * - Car horn: https://www.youtube.com/watch?v=C3ddYv-hApE
 * - Ambient street sounds: https://www.youtube.com/watch?v=F6S77At4Qvk
 * - Night time sound: https://www.youtube.com/watch?v=OXXoI2MF-Gs
 * - Police Spawn sound: https://www.youtube.com/watch?v=M_EST85_4kQ
 * - Pedestrian help sound: https://www.youtube.com/watch?v=EQZcXCFdRQM
 * - Vroom sound effect for robber car: https://www.youtube.com/watch?v=HjpBzQ_oWwU
 * - Thank you sound effect for healing a pedestrian: https://www.youtube.com/watch?v=cbX8fdVvMvs
 * Code: 
 * - Mr. Cohen's bug simulation code (Bug class, Flower class, DeadBug class)
 * 
 * Bugs:
 * - Sometimes I get an error for spawning a cop. I'm not sure why and it doesn't happen often.
 * - If a cop is arresting a robber it makes it transparent. If the cop dies as its doing this, the robber will remain half transparent.
 * - Robbers continue to rob a pedestrian even while being arrested.
 * - Robbers and cops move through vehicles
 * - Sometimes a car will lane change and it will change lanes into another car, but it doesn't happen often.
 */
/**
...
public class VehicleWorld extends World
{
    ...
    public static Color GREY_STREET = new Color (50, 50, 50);
    ...
    {    
        ...
        setPaintOrder (Night.class, Vehicle.class, Pedestrian.class); // Commented out to use Z-sort instead
        ...
        setBackground (background);
        ambientSound = new GreenfootSound("streetsounds.wav");
        ambientSound.setVolume(35);
    }
    public void started(){
    ...
    private void spawn () {
        ...
        if (actCount % 1000 == 0 ) {
            ...
            isNight = true; 
            for (int i = 0; i < 10; i++) {
                int xSpawnLocation = Greenfoot.getRandomNumber (700) + 100;
                boolean spawnAtTop = Greenfoot.getRandomNumber(2) == 0 ? true : false;
                if (spawnAtTop){
                    addObject (new Robber (1), xSpawnLocation, TOP_SPAWN);
                } else {
                    addObject (new Robber (-1), xSpawnLocation, BOTTOM_SPAWN);
                }
            }
        }
        //spawn bomb
        if (getObjects(Bomb.class).size() == 0 && getObjects(Vehicle.class).size() > 8){
            addObject (new Bomb(), getWidth()/2, 0);
        }
        // Chance to spawn a vehicle
        if (Greenfoot.getRandomNumber (laneCount * 4) == 0){
            ...
            if (!laneSpawners[lane].isTouchingVehicle()){
                ...
                if (vehicleType == 0){
                    ...
                    addObject(new RobberCar(laneSpawners[lane]), 0, 0); 
                } else if (vehicleType == 4) {
                    addObject(new PoliceCar(laneSpawners[lane]), 0, 0); 
                }
            }
        }
        ...
        if (Greenfoot.getRandomNumber (80) == 0){
            ...
            boolean spawnAtTop = Greenfoot.getRandomNumber(2) == 0 ? true : false;
            if (spawnAtTop){
                addObject (new Man (1), xSpawnLocation, TOP_SPAWN);
            } else {
                addObject (new Man (-1), xSpawnLocation, BOTTOM_SPAWN);
            }
        }
        
        // Chance to spawn a Robber
        if (Greenfoot.getRandomNumber (robberSpawnRate) == 0){
            int xSpawnLocation = Greenfoot.getRandomNumber (600) + 100; // random between 99 and 699, so not near edges
            boolean spawnAtTop = Greenfoot.getRandomNumber(2) == 0 ? true : false;
            if (spawnAtTop){
                addObject (new Robber (1), xSpawnLocation, TOP_SPAWN);
     ...
     */
    public void setNight (boolean checkNight) {
        isNight = checkNight; 
    }
